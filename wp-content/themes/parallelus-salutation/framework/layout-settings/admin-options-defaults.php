...
// DEFAULT DESIGN SETTINGS
	...
	echo '<table class="form-table">';
		...
		if (bbPress_plugin_is_active()) {
			...
			$select = $select_layout;
			$comment = __('The default layout for your bbPress forum and topic pages.', THEME_NAME);
			$comment = $layout_admin->format_comment($comment);
			$row = array(__('bbPress', THEME_NAME), $layout_admin->settings_select('layout,bbpress', $select) . $comment);
			$layout_admin->setting_row($row);
