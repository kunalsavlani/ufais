		...
		switch(intval($pagenavi_options['style'])) {
			...
			case 1:
				...
				if ($start_page >= 2 && $pages_to_show < $max_page) {
					...
					echo '<li class="pagebutton"><a href="'.clean_url(get_pagenum_link()).'" class="first" title="'.$first_page_text.'">'.$first_page_text.'</a></li>';
					if (!empty($pagenavi_options['dotleft_text'])) {
						echo '<li class="extend"></li>';
					}
				}
				$larger_page_start = 0;
				foreach($larger_pages_array as $larger_page) {
					if ($larger_page < $start_page && $larger_page_start < $larger_page_to_show) {
						...
						echo '<li class="pagebutton"><a href="'.clean_url(get_pagenum_link($larger_page)).'" class="page" title="'.$page_text.'">'.$page_text.'</a></li>';
						$larger_page_start++;
					}
				}
				$previouspostlink = get_previous_posts_link($pagenavi_options['prev_text']);
				
				if($previouspostlink) {
				echo '<li class="pagebutton">'.$previouspostlink.'</li>';
				}
				
				for($i = $start_page; $i  <= $end_page; $i++) {						
					if ($i == $paged) {
						$current_page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['current_text']);
						echo '<li class="pagebutton active">'.$current_page_text.'</li>';
					} else {
						$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['page_text']);
						echo '<li class="pagebutton"><a href="'.clean_url(get_pagenum_link($i)).'" class="page" title="'.$page_text.'">'.$page_text.'</a></li>';
					}
				}
				$nextpostlink = get_next_posts_link($pagenavi_options['next_text'], $max_page);
				
				if($nextpostlink) {
				echo '<li class="pagebutton">'.$nextpostlink.'</li>';
				}
				...
				if ($end_page < $max_page) {
					...
					echo '<li class="pagebutton"><a href="'.clean_url(get_pagenum_link($max_page)).'" class="last" title="'.$last_page_text.'">'.$last_page_text.'</a></li>';
				}
				break;
			// Dropdown
			case 2;
...
function pagenavi_init() {
	...
	$pagenavi_options = array(
		'pages_text' => __('Page %CURRENT_PAGE% of %TOTAL_PAGES%','NorthVantage'),
		'current_text' => '%PAGE_NUMBER%',
		'page_text' => '%PAGE_NUMBER%',
		'first_text' => __('&laquo; &laquo;','NorthVantage'),
		'last_text' => __('&raquo; &raquo;','NorthVantage'),
		'next_text' => __('&raquo;','NorthVantage'),
		'prev_text' => __('&laquo;','NorthVantage'),
		'dotright_text' => __(' ','NorthVantage'),
		'dotleft_text' => __(' ','NorthVantage'),
		'style' => 1,
		'num_pages' => 5,
		'always_show' => 0,
