...
if ( ! function_exists( 'bp_enqueue_defaults_init' ) ) :
	function bp_enqueue_defaults_init() {
		...
		theme_register_css( 'buddypress', $cssPath.'buddypress.css' );
...
if ( ! function_exists( 'bp_theme_avatar_url' ) ) :
	function bp_theme_avatar_url( $width, $height, $use_function = '', $avatarURL = '' ) {
		...
		// Update: BP v1.6, strip only SRC from returned IMG tag.
		$matches = array();
		preg_match( '/src="([^"]*)"/i', $avatarURL, $matches ) ;
		if ( count($matches) ) {
			$avatarURL = $matches[1];
		}
...
if ( ! function_exists( 'custom_bp_activity_action' ) ) :
	function custom_bp_activity_action() {
		...
		if ( bp_activity_user_can_delete() ) { 
			$content = str_replace('</p>', bp_get_activity_delete_link() . '</p>', $content);
		}
		echo $content;
	}
endif;
...
function bp_nav_menu_item_meta_box() {
	...
	<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
		<ul id="posttype-<?php echo $post_type_name; ?>-tabs" class="posttype-tabs add-menu-item-tabs">
			<li <?php echo ( 'all' == $current_tab ? ' class="tabs"' : '' ); ?>><a class="nav-tab-link" href="<?php if ( $nav_menu_selected_id ) echo esc_url(add_query_arg($post_type_name . '-tab', 'all', remove_query_arg($removed_args))); ?>#<?php echo $post_type_name; ?>-all"><?php _e('All Links'); ?></a></li>
		</ul>
...
function add_bp_menu_meta_box() {
	...
	add_meta_box( "add-bpMenu", __( 'BuddyPress Links', 'buddypress' ), 'bp_nav_menu_item_meta_box', 'nav-menus', 'side', 'default', $bp_nav_menu_items );
}
