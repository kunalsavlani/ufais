...
// Get the user's name for the activity
// ------------------------------------------------------------------------

if ( !function_exists( 'theme_bp_get_activity_username' ) ) :
	function theme_bp_get_activity_username() {
		global $activities_template;

		$name = $activities_template->activity->display_name;
		
		if (!$name) {
			$name = $activities_template->activity->user_nicename;
		}
	...
	<article>
		...
		<div class="activity-content item-content-container <?php if ( !bp_get_activity_content_body() ) : echo 'no-inner-content'; endif; ?>" id="ytivitca-<?php bp_activity_id() ?>">
			...
			<header class="comment-header">
				...
				$user_nicename =  theme_bp_get_activity_username(); // $GLOBALS['activities_template']->activity->user_nicename; // bp_core_get_user_displayname( $GLOBALS['activities_template']->activity->user_id );
				?>
				<h4 class="poster-name"><a href="<?php bp_activity_user_link() ?>" title="<?php _e( 'Go to '.$user_nicename. '\'s member page.', 'buddypress' ); ?>"><?php echo $user_nicename ?></a></h4>
